...
it.each([false, true])('closes a disposed terminal and reopens without old input or proof (collapsed=%s)', async collapsed => {
  const { transport, terminal, view } = await bench()
  const dispose = vi.spyOn(terminal, 'dispose')
  act(() => { terminal.input('a', true); terminal.input('bc', true) })
  fireEvent.click(screen.getByRole('button', { name: 'Reset Terminal' }))
  if (collapsed) fireEvent.click(screen.getByRole('button', { name: /^(Open terminal|Terminal)$/ }))
  await transport.close(4002)
  expect(dispose).toHaveBeenCalledTimes(1)
  expect(view.container.querySelector('.xterm')).toBeNull()
  const toggle = screen.getByRole('button', { name: 'Open terminal' })
  expect(toggle.getAttribute('aria-expanded')).toBe('false')
  expect(screen.queryByRole('status')).toBeNull()
  await new Promise(resolve => setTimeout(resolve, 300))
  expect(transport.attach).toHaveBeenCalledTimes(1)
  await transport.receive({ type: 'terminal.output', version: 1, generation: 1, sequence: 1, output: 'stale' }, 0)
