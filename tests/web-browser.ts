...
export async function openWorkspace(page: Page, origin: string, workspace: string) {
  ...
  const later = page.getByRole('button', { name: /^(稍后配置|Configure later)$/i, exact: true })
  if (await later.isVisible()) await later.click()
  await page.getByRole('button', { name: /^(添加工作区|Add workspace)$/, exact: true }).click()
...
export async function prompt(page: Page, text: string) {
  ...
  await page.getByRole('button', { name: /^(发送消息|Send message)$/, exact: true }).click()
}
