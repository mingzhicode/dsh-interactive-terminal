...
export type ServerFrame = { version: 1; generation: number } & (
  | { type: 'terminal.snapshot'; snapshot: ScreenSnapshot; scrollbackLines: number }
  | { type: 'terminal.attached'; mode: 'controller'; resume: string; maxInputBytes: number }
  | { type: 'terminal.attached'; mode: 'readonly'; reason: 'readonly' | 'controller-busy' | 'invalid-resume'; maxInputBytes: number }
  | { type: 'terminal.output'; output: string; sequence: number }
  | { type: 'terminal.status'; status: TerminalStatus; queueStatus: QueueStatus; holder: OperationKind | null; takeoverId: TakeoverId | null; pendingCount: number }
  | { type: 'human.granted' }
  | { type: 'human.queued'; position: number }
  | { type: 'human.revoked' }
  | { type: 'terminal.error'; code: 'operation-failed' | 'transport-failed'; message: string }
  | { type: 'heartbeat' }
)

/** A frame accepted by the authoritative terminal host. */
export type ClientFrame = { version: 1; generation: number } & (
  | { type: 'human.begin'; input: string }
  | { type: 'human.takeover'; target: TakeoverId }
  | { type: 'human.input'; input: string }
  | { type: 'human.cancel' }
  | { type: 'terminal.reset'; confirmed: true }
  | { type: 'heartbeat' }
)
